...
model_prefix <- ct
wk="/data/l2_bioinfo1/liq17/MethExp/Meth_UVA/Methy_CellTypes/TFTWAS_RES_add_corr/"

######II. Collect all model summary files [ensure one ENSG name]
model_summary <- matrix(integer(0), nrow = 0, ncol = 7) %>% as.data.frame()

model_weights <- matrix(integer(0), nrow = 0, ncol = 6) %>% as.data.frame()
names(model_weights) <- c("rsid","cpG","beta","ref","alt") #cpG     rsid    varID   ref     alt     beta

model_covars <- matrix(integer(0), nrow = 0, ncol = 4) %>% as.data.frame()
names(model_covars) <- c('cpG', 'rsid1', 'rsid2', 'corvarianceValues')
